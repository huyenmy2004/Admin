...
if (isset($_GET['search-prodName']) || isset($_GET['search-status']))
    $orders = $order->getData($_GET['search-prodName'], $_GET['search-status']);
else {
    $orders = $order->getData(null, null);
}
$page = isset($_GET['page']) ? $_GET['page'] : 1;
$pageSize = isset($_GET['pageSize']) ? $_GET['pageSize'] : 10;
if (count($orders) <= ($page - 1) * $pageSize) {
    $page = 1;
}
                    ...
                    <div class="relative ml-10 w-[20%]">
                        <div class="relative" onclick="showDroplist('status-droplist')" id="status-search">
                            ...
                            <input type="text" readonly
                                value="<?= isset($_GET['search-status']) ? ($_GET['search-status'] == 1 ? "Chờ xác nhận" : ($_GET['search-status'] == 2 ? "Đã xác nhận" : ($_GET['search-status'] == 3 ? "Đang giao hàng" : ($_GET['search-status'] == 4 ? "Đã giao hàng" : ($_GET['search-status'] == 5 ? "Đã huỷ" : "Tất cả"))))) : "Tất cả" ?>"
                            ...
                            <div class="absolute flex flex-col bg-white py-2 rounded-[6px] border border-[#d8d8d8] text-[13px] hidden w-[280px]"
                                ...
                                <span class="hover:bg-gray-100 px-[20px] py-[3px] text-[#505050]"
                                    onclick="select('status-search', 1, 'Chờ xác nhận')">Chờ xác nhận</span>
                                <span class="hover:bg-gray-100 px-[20px] py-[3px] text-[#505050]"
                                    onclick="select('status-search', 2, 'Đã xác nhận')">Đã xác nhận</span>
                                <span class="hover:bg-gray-100 px-[20px] py-[3px] text-[#505050]"
                                    onclick="select('status-search', 3, 'Đang giao hàng')">Đang giao hàng</span>
                                <span class="hover:bg-gray-100 px-[20px] py-[3px] text-[#505050]"
                                    onclick="select('status-search', 4, 'Đã giao hàng')">Đã giao hàng</span>
                                <span class="hover:bg-gray-100 px-[20px] py-[3px] text-[#505050]"
                                    onclick="select('status-search', 5, 'Đã huỷ')">Đã huỷ</span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="flex flex-col px-[50px] py-[25px]">
                ...
                <div class="mt-[20px] text-[#505050]">
                    <div class="grid grid-cols-9 border-b pb-2">
                        ...
                        <span class="text-[13px] font-[600]">Ngày đặt</span>
                        <span class="text-[13px] col-span-2 font-[600]">Tên khách hàng</span>
                        <span class="text-[13px] font-[600]">Số điện thoại</span>
                        <span class="text-[13px] col-span-2 font-[600]">Địa chỉ</span>
                        <span class="text-[13px] font-[600]">Trạng thái</span>
                        <span class="text-[13px] font-[600]">Tổng tiền</span>
                    </div>
                    <?php
                    $i = ($page - 1) * $pageSize;
                    foreach (array_slice($orders, ($page - 1) * $pageSize, $pageSize) as $order): ?>
                        <div class="grid grid-cols-9 py-3 border-b cursor-pointer"
                            onclick="window.location.href = 'http://localhost/PharmaDI-Admin/order/order-detail.php?orderId=<?= $order['orderId'] ?>'">
                            <span class="text-[13px] truncate ">
                                <?= $i = $i + 1 ?>
                            </span>
                            <span class="text-[13px] whitespace-nowrap overflow-hidden w-[75px]">
                                <?= $order['orderDate'] ?>
                            </span>
                            ...
                            <span class="text-[13px] col-span-2 overflow-x-hidden whitespace-nowrap">
                                <?= $order['cusAddress'] ?>
                            </span>
                            <span class="text-[13px] truncate">
                                <?= $order['orderStatus'] == 1 ? "Chờ xác nhận" : ($order['orderStatus'] == 2 ? "Đã xác nhận" : ($order['orderStatus'] == 3 ? "Đang giao hàng" : ($order['orderStatus'] == 4 ? "Đã giao hàng" : "Đã huỷ"))) ?>
                            </span>
                            <span class="text-[13px] truncate">
                    ...
                    <div class="flex items-center justify-end mt-6">
                        ...
                        <div class="flex items-center pl-[10px]">
                            <svg class="cursor-pointer" width="16" height="16"
                                onclick="document.getElementById('page-order').value = <?= 1 ?>; submitForm('form-order-search')"
                            ...
                            <svg class="cursor-pointer" width="16" height="16"
                                onclick="document.getElementById('page-order').value = <?= $page - 1 ?>; submitForm('form-order-search')"
                            ...
                            <div class="w-[100px] overflow-hidden relative h-[20px]">
                                <div class="flex cursor-pointer" id="container-slide" style="--transitionto:0px">
                                    <input type="text" class="hidden" name='page' id='page-order' value='<?= $page ?>'>
                                    <?php
                                    for ($i = 0; $i <= floor(count($orders) / $pageSize); $i++): ?>
                                        <span
                                            class="text-[13px] px-1 min-w-[20px] min-h-[20px] rounded-full flex justify-center items-center <?php if ($page == $i + 1)
                                                echo 'bg-[#d8d8d8]' ?>"
                                                onclick="document.getElementById('page-order').value = <?= $i + 1 ?>; submitForm('form-order-search')">
                                            <?= $i + 1 ?>
                                        </span>
                                    <?php endfor ?>
                                </div>
                            ...
                            <svg class="cursor-pointer" width="16" height="16"
                                onclick="document.getElementById('page-order').value = <?= floor(count($orders) / $pageSize) + 1 ?>; submitForm('form-order-search')"
                                viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd"
